...
font_size['values'] = tuple(range(8,85))
font_size.current(4)
...
def change_font(enent=None):
    global current_font_family
    current_font_family = font_family.get()
    text_editor.configure(font=(current_font_family, current_font_size))

def change_font_size(enent=None):
...
def open_file(event=None):
    global url
    url = filedialog.askopenfilename(initialdir=os.getcwd(), title='Select File', filetypes=(('Text Fiel', '*.txt'), ('All File', '*.*')))
    try:
...
def save_file(enent=None):
    ...
    try:
        if url:
            ...
            url = filedialog.asksaveasfile(mode= 'w', defaultextension = '.txt', filetypes=(('Text Fiel', '*.txt'), ('All File', '*.*')))
...
def find_func(even=None):
    ...
    ## labels
    text_find_lebel = ttk.Label(find_frame, text='Find : ')
    text_replace_lebel = ttk.Label(find_frame, text='Replace : ')
    ...
    ## label grid
    text_find_lebel.grid(row=0, column=0, padx=4, pady=4)
    text_replace_lebel.grid(row=1, column=0, padx=4, pady=4)
...
def agent_assist_dialog(event=None):
    ...
    def run_action(action):
        ...
        if action == 'summarize':
            ...
            label = 'Clarity rewrite complete'
        elif action == 'bullets':
            result = to_bullet_points(source)
            label = 'Bulleted version ready'
        else:
            result = source
            label = 'Result ready'
    ...
    def copy_result():
        ...
        status.config(text='Result copied to clipboard')
    ...
    def apply_to_editor():
        ...
        status.config(text='Applied to editor')
        window.destroy()
    ...
    ttk.Button(controls, text='Copy Result', command=copy_result).pack(side=tk.RIGHT, padx=4)
